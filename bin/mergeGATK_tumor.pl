...
sub merge {
    ...
    my @info_fields_to_comma_merge = ("NONE");
	...
	if( @agg ) {
	    ...
	    my $agg_rd_first = ( split /,/, $agg_info{FOLD_CHANGE} )[0];
	    
	    if( $data[0] eq $agg[0] and 
		$dist/abs($info{SVLEN}) < 0.1 and 
		$dist/abs($agg_info{SVLEN}) < 0.1 and
		$dist < 50000 and
		abs($agg_rd_first - $info{FOLD_CHANGE}) < 0.2 and 
		...
		$var .= "\t";
		my @info_fields;
		for ("END", "SVLEN", "SVTYPE","FOLD_CHANGE_LOG","FOLD_CHANGE","PROBES") {
            if ($_ eq "FOLD_CHANGE_LOG" || $_ eq "FOLD_CHANGE") {
                my @list = split(',',$agg_info{$_});
                my $sum = 0;
                foreach my $val (@list) {
                    $sum = $sum + $val;
                }
                my $average = $sum/scalar(@list);
                push @info_fields, "$_=$average";
            }
            elsif ($_ eq "PROBES") {
                my @list = split(',',$agg_info{$_});
                my $sum = 0;
