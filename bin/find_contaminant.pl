...
my %opt = ();
GetOptions( \%opt, 'vcf=s', 'case-id=s', 'normal', 'assay=s', 'detect-level=s' );
my $assay = $opt{assay};
...
while ( my $v = $vcf->next_var() ) {
    ...
    my $t_VAF; my $t_VD; my $t_DP;
    my $n_VAF; my $n_VD; my $n_DP;
	...
	my $var = $v->{CHROM}."\t".$v->{POS}."\t".$v->{REF}."\t".$v->{ALT};
	...
	for my $gt (@{$v->{GT}}) {
		if ($gt->{_sample_id} eq $id) {
			$t_VD = $gt->{$assay_info{$assay}->{ADfield}};
			my @ads = split(/,/,$t_VD);
			my $min = min(@ads);
			$t_DP = $gt->{DP};
			if ($assay_info{$assay}->{ADfield} eq 'VD') {
				$t_VAF = $gt->{VAF};
			}
			else {
				if ($t_DP == 0) {
			...
			$n_VD = $gt->{$assay_info{$assay}->{ADfield}};
			my @ads = split(/,/,$t_VD);
			my $min = min(@ads);
			$n_DP = $gt->{DP};
			if ($assay_info{$assay}->{ADfield} eq 'VD') {
...
my ($af_at_highpoint,$bin_at_highpoint) = find_heterozygous_peak($num_bins,$num_vars_bin);
...
sub find_homozygous_peak {
	...
	for (my $i = $first_peak+1; $i <= $num_bins-1; $i++) {
		if ($distri{$i}{COUNT} < $distri{$i+1}{COUNT} && $distri{$i}{COUNT} > $homo_highpoint ) {
			$homo_highpoint = $distri{$i+1}->{COUNT};
			$af_at_homo = $distri{$i+1}->{MEAN};
			$bin_at_homo_highpoint = $i+1;
		}
	}
	if ($af_at_homo/$af_at_hetero <= 2.2 && $af_at_homo/$af_at_hetero >= 1.8) {
...
sub find_heterozygous_peak {
	...
	my $binsize_cutoff = $assay_info{$assay}->{cutoff};
	my @xdata;
	my @ydata;
	foreach my $af_c (sort { $a <=> $b } keys %distri) {
		...
		if ($distri{$af_c}->{COUNT}/$mean_bincount >= 3.5) {
			if ($distri{$af_c}->{COUNT} > $highpoint && $distri{$af_c}->{COUNT} > $binsize_cutoff) {
				$highpoint = $distri{$af_c}->{COUNT};
				$af_at_highpoint = $distri{$af_c}->{MEAN};
				$bin_at_highpoint = $af_c;
			}
		}
		print DIST $af_c."\t".$distri{$af_c}->{COUNT}."\t".$distri{$af_c}->{MEAN}."\n";
	}
	close DIST;
	my @data;
