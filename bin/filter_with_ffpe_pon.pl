...
while ( my $var = $vcf->next_var() ) {
	...
	if( $pon->{$varid} ) {
		...
		if( $T_VAF < $p->{mean_vaf} + $p->{stdev_vaf}*2 ) {
		    push @filters, "FAIL_FFPE_PON_$vc";
		    $remove_pass = 1;
		}
		else {
		    push @filters, "WARN_FFPE_PON_$vc";
		}
	    }
	    add_info($var, "PON_FFPE_NUM_$vc", $p->{num_non_germ}."/".$p->{total});
	    add_info($var, "PON_FFPE_VAFS_$vc", $p->{all});
...
sub print_header {
    ...
    system("zgrep ^## $file");
    for my $vc (@$vcs) {
	print "##FILTER=<ID=WARN_FFPE_PON_$vc,Description=\"Warning, exists in PON for $vc\">\n";
	print "##FILTER=<ID=FAIL_FFPE_PON_$vc,Description=\"Failed, exists in PON for $vc\">\n";

	print "##INFO=<ID=PON_FFPE_NUM_$vc,Number=.,Type=String,Description=\"Number of PON samples variants exists in, for $vc\">\n";
	print "##INFO=<ID=PON_FFPE_VAFS_$vc,Number=.,Type=String,Description=\"VAFs for PON variants, for $vc\">\n";
    }
